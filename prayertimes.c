...
int main(int argc, char* argv[])
{
	...
	for (;;)
	{
		...
		switch (c)
		{
			case 0:
				...
				{
					case DHUHR_MINUTES:
						set_dhuhr_minutes(arg);
						break;
					case MAGHRIB_MINUTES:
						set_maghrib_minutes(arg);
						break;
					case ISHA_MINUTES:
						set_isha_minutes(arg);
						break;
					case FAJR_ANGLE:
						set_fajr_angle(arg);
						break;
					case MAGHRIB_ANGLE:
						set_maghrib_angle(arg);
						break;
					case ISHA_ANGLE:
						set_isha_angle(arg);
						break;
			...
			case 'd':		// --date
			{
				struct tm* new_date = getdate(optarg);
				if (!new_date)
				{
	...
	if (isnan(timezone))
		timezone = get_effective_timezone(date);

	double times[TimesCount];
	char time_buffer[20]; // needed to hold time string
	...
	for (int i = 0; i < TimesCount; ++i)
		printf("%8s : %s\n", TimeName[i], float_time_to_time24(times[i], time_buffer));
	return 0;
}
